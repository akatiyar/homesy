...
long socketWrite(short socketHandle, const char *sendBuffer, unsigned int sendBufferSize)
{
    long status = 0;
...
//    }
  ...
  //  	DEBUG_PRINT("^");
        if((sendBufferSize-sent_data_size) >= 1024)
            ...
            size_to_send = (sendBufferSize-sent_data_size);

        status = send(socketHandle, sendBuffer+sent_data_size, size_to_send, 0);
